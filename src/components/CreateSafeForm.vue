<template>
  <div>
    ...
    <div :class="[{ hidden: step !== 1 }]">
      ...
      <div class="controls">
        <router-link to="/app"> Cancel </router-link>
        <button @click="goToStep(2)" :disabled="!address">Continue</button>
      </div>
    </div>
    ...
    <div :class="[{ hidden: step !== 2 }]">
      ...
      <div class="controls">
        ...
        <button @click="goToStep(3)" :disabled="!isSignerFormFilled">
          Continue
        </button>
      </div>
    </div>
    ...
    <div :class="[{ hidden: step !== 3 }]">
      ...
      <br />
      <p>
        You're about to create a new Multisig safe on Aeternity and will have to
        confirm a transaction with your currently connected wallet. The creation
        will cost approximately 0.00000556 AE. The exact amount will be
        determined by your wallet. Approximate creation time: 5 mins
      </p>
      <br />

      <div class="controls">
        <button @click="goToStep(2)">back</button>
...
async function createSafe() {
  const { contractId, gaAccountId } = await initSafe(
    initSigners.value,
    initConfirmationsRequired.value
  );
